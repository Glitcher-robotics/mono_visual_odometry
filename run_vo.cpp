...
int main(int argc, char **argv){
    ...
    assert(checkInputArguments(argc, argv)); // file well done?
    ...
    if (IS_DEBUGGING){          // Read certain images specified below.
        string folder = "/home/glitcher/Desktop/fd/mono_visual_odometry/Monocular-Visual-Odometry-Data/data/test_data/";
        vector<string> tmp{
    ...
    for (int img_id = 0; img_id <std::min(max_num_imgs_to_proc, (int)image_paths.size()); img_id++){
        ...
        vo -> addFrame(frame); //This is core of VO !!!!!!!
...
bool drawResultByOpenCV(const cv::Mat &rgb_img, const vo::Frame::Ptr frame, const vo::VisualOdometry::Ptr vo){
    ...
    else{ // draw all & inlier keypoints in the current frame
    ...
    // Save to file
    contexpr bool kIsSaveImageToDisk = true;
    if (kIsSaveImageToDisk)
    {
        const string str_img_id = basics::int2str(img_id, 4);
        imwirte(output_folder + "/" + str_img_id + ".png", img_show);
    }
                    ...
                    display::PclViewer::Ptr pcl_displayer)
{
    ...
    if (is_draw_true_traj){
        ...
        // The reason is: we need scale the truth pose to be same as estimated pose, so we can make comparison between them.
        // (And the underlying reason is that Mono SLAM cannot estimate depth of point.)
        if (vo->isInitialized()){
