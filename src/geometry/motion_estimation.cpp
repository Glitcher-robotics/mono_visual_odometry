    ...
    const bool is_motion_cam2_to_cam1)
{
    ...
    {
        printResult_estiMotionByEssential(essential_matrix,
...
void helperEstiMotionByEssential(
    ...
    const cv::Mat &K,
    cv::Mat &R, cv::Mat &t,
    vector<cv::DMatch> &inliers_matches,
    bool is_print_res)
{
...
void helperEvalEppiAndTriangErros(
    const vector<cv::KeyPoint> &keypoints_1,
    const vector<cv::KeyPoint> &keypoints_2,
    ...
    bool is_print_res)
{
    ...
    for (int i = 0; i < num_solutions; i++)
    {
        ...
        for (int idx_inlier = 0; idx_inlier < num_inlier_pts; idx_inlier++)
        {
            ...
            double dist1 = calcErrorSqure(p1, pts2dc1), dist2 = calcErrorSquare(p2, pts2dc2);
...
// Check [Epipolar error] and [Triangulation result] for each common inlier in both E and H
    ...
    const vector<cv::Mat> &list_R, const vector<cv::Mat> &list_t, const cv::Mat &K)
{
    ...
    cout << "\n---------------------------------" << endl;
    cout << "Check [Epipolar error] and [Triangulation result]" << endl;
    ...
    for (int i = 0; i < num_points && cnt < kMaxNumPtsToCheckAndPrint; i++)
    {
        ...
        cv::Point2f p_cam1 = pts_on_np1[i]; // point pos on the normalized plaen
