                     ...
                     double rot_axis_x, double rot_axis_y, double rot_axis_z)
{
    ...
    viewer_->addCoordinateSystem(LEN_COORD_AXIS, "fixed world frame"); //import function
    ...
    point_clouds[pc_cam_traj_ground_truth] = addPointCloud(viewer_, pc_cam_traj, 7);
...
{
    // Add a line between new_pose and old_pos. Also, add a point.
    if (cnt_cam > 0)
    {
...
// }
// void deletePoints0fCloud(const string &cloud_name)
// {
//     assert(point_clouds.find(cloud_name)!=point_clouds.end());
