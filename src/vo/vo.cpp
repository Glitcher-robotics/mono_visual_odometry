...
VisualOdeometry::VisualOdometry() : map_(new (Map))
{
    vo_state_ = BLANK;
}
...
void VisualOdometry::estimateMotionAnd3DPoints_()
{
    ...
    vector<vector<cv::Point3f>> sols_pts3d_in_caml_by_triang;
    bool is_print_res = false, is_frame_cam2_to_caml = true;
    ...
    if (N < 20)
    {
        printf("After remove bad triang, only %d pts. It's too few ...\n", N);
        return;
    }
...
void VisualOdometry::retainGoodTriangulationResult_()
{
    ...
    // vector<cv::Point3f> pts3d_in_curr: curr_ -> inliers_pts3d_;
    // 3. generate this:
    // vector<cv::DMatch> inliers_mathces: curr_ -> inliers_matches_for_3d_;
...
bool VisualOdometry::checkLargeMoveForAddKeyFrame_(Frame::Ptr curr, Frame::Ptr ref)
{
    ...
    // static const double min_rotation_angle_between_two_keyframes = basics::Config::get<double>("min_rotation_angle_between_two_keyframes");
    ...
    // Statisfy each one will be a good keyframe
...
void VisualOdometry::callBundleAdjustment_()
{
    ...
    // Set params
    const int kTotalFrames = frames_buff.size();
    ...
    vector<cv::Point3f *v> v_pts_3d_only_in_curr;
    ...
    if (1)
    {
        optimization::bundleAdjustment(
            ...
            um_pts_3d_in_prev_frmaes, v_camera_poses,
            information_matrix,
...
void ViusalOdometry::addKeyFrame_(Frame::Ptr frame)
{
    ...
    for (int i = 0; i < inliers_matches_for_3d.size(); i++)
    {
        ...
        else // Not triangulated before. Create and push to map.
        {
            ...
            MapPoint::Ptr map_point(new MapPoint( // createMapPoint
                ...
                basics::getNormalizeMat(basics::point3f_to_mat3x1(world_pos) - curr_->getCamCenter()),   // view direction of the point
                kpts_colors[pt_idx][0], kpts_colors[pt_idx][1], kpts_colors[pt_idx][2]
                ));
