...
void VisualOdometry::addFrame(Frame::Ptr frame)
{
    ...
    if (vo_state_ == BLANK)
    {
        curr_->T_w_c_ = cv::Mat::ey(4, 4, CV_64F);
        vo_sate_ = DOING_INITIALIZATION;
    ...
    else if (vo_state_ == DOING_TRACKING)
    {
        ...
        else // pnp good
        {
            ...
            // -- Insert a keyframe if motion is large. Then, triangulate more points
            if (checkLargeMoveForAddKeyFrame_(curr_, ref_))
            {
                ...
                curr_->inliers_mathces_with_ref_ = geometry::helperFindInlierMatchesByEpipolarCons(
                ...
                printf("For triangulation: Matches with prev keyframe: %d; Num inliers: %d \n",
                       (int)curr_->mathces_with_ref_.size(), (int)curr_->inliers_matches_with_ref_.size());
